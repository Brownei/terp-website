...
GENESIS=~/.terp/config/genesis.json
CONFIG_TOML=~/.terp/config/config.toml

# ── Patch chain-id ────────────────────────────────────────────────────────────
# The bootstrap script initialises with 120u-1; override to 120u-2.
jq '.chain_id = "120u-2"' "$GENESIS" > "$GENESIS.tmp" && mv "$GENESIS.tmp" "$GENESIS"
echo "post_init_chain2: chain_id → 120u-2"
